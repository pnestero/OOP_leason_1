...
@pytest.fixture
def tests1_product():
    return Product(name="Банан", description="Чтобы съесть", price=123.12, quantity=2)


@pytest.fixture
def tests2_product():
    return Product(name="Чипсы", description="Закуска", price=200, quantity=3)
...
@pytest.fixture
def tests2_category():
    return Category(name="Телефон", description="program developer", products=[])
...
def tests2_category_average_price():
    return Category(
        ...
        init_products=[
            Product(name="Банан", description="Чтобы съесть", price=0, quantity=1)
        ],
    )
